...
all_data2 <- rbind(mitubushi,daiwa,smbc)
```
...
mitsubishi_UFJ_koskusai <- mitsubishi_UFJ_koskusai[order(mitsubishi_UFJ_koskusai$sales_co, mitsubishi_UFJ_koskusai$fund_code),]
...
nomura  <- nomura[order(nomura$sales_co, nomura$fund_code),]
kable(nomura)
```
...
pikute <- pikute[order(pikute$sales_co, pikute$fund_code),]
kable(pikute)
```
...
DS  <- DS[order(DS$sales_co, DS$fund_code),]
kable(DS)
```
